...
setup(args, 'training')
#对训练或者测试数据分别数据的预先处理；
#RandomRotion(10)随机对图像进行旋转（-10，10）之间；
#RandomIntensityChange对intensity强度随机进行改变；
#RandomFlip(0)随机进行水平翻转；
...
def main():
    ...
    if args.dataname in ['BRATS2021', 'BRATS2020', 'BRATS2018']:
        ...
        exit(0)
    model = mmformer.Model(num_cls=num_cls) #创建模型对象；
    print (model) #打印模型结构信息；
    model = torch.nn.DataParallel(model).cuda() #将模型加载到cuda上；
    ...
    for epoch in range(args.num_epochs):
        step_lr = lr_schedule(optimizer, epoch) #根据epoch进行学习率的调整；
        ...
        for i in range(iter_per_epoch):
            ...
            try:
                ...
                train_iter = iter(train_loader)
                data = next(train_iter)
            x, target, mask = data[:3] #加载输入图像数据、真值标签、对应的随机缺失模态掩膜
            x = x.cuda(non_blocking=True)
            target = target.cuda(non_blocking=True)
            mask = mask.cuda(non_blocking=True)

            model.module.is_training = True #将模型模式设置为训练模式
            fuse_pred, sep_preds, prm_preds = model(x, mask) #输入到模型中进行预测
